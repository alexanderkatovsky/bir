...
{
    uint16_t cksum = tcp_checksum((uint8_t *)TCP_HDR(packet), IP_HDR(packet)->ip_src.s_addr,
...
void tcp_handle_incoming_not_for_us(struct sr_packet * packet)
{
    ...
    uint16_t src_port = ntohs(TCP_HDR(packet)->tcph_srcport);
    uint16_t dst_port = ntohs(TCP_HDR(packet)->tcph_destport);
