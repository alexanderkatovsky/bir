...
void ospf_send_hello(struct sr_instance * sr, struct sr_vns_if * vnsif)
{
    int len = sizeof(struct sr_ethernet_hdr) + sizeof(struct ip) +
        sizeof(struct ospfv2_hdr) + sizeof(struct ospfv2_hello_hdr);

    uint8_t * packet = (uint8_t *)malloc(len);
    ip_construct_eth_header(packet,0,vnsif->addr,ETHERTYPE_IP);
    ip_construct_ip_header(packet,len,0,OSPF_MAX_LSU_TTL,IP_P_OSPF,vnsif->ip,htonl(OSPF_AllSPFRouters));
    ospf_construct_ospf_header(packet,OSPF_TYPE_HELLO,len,htonl(ROUTER(sr)->rid),ROUTER(sr)->aid);
    ospf_construct_hello_header(packet,vnsif->mask,OSPF_DEFAULT_HELLOINT);
    if(sr_integ_low_level_output(sr,packet,len,vnsif->name) == -1)
    {
